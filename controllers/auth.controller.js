  ...
  try {
    ...
    if (username) {
      return res
        ...
        .json({
          success: false,
          message: "User with that username already exists in our database",
        });
    }
    if (userEmail) {
      return res
        ...
        .json({
          succes: false,
          message:
            "User with that email address is already exists in out data base",
        });
    }
    //password encryption
...
export const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!isPasswordMatch) {
      return res
        .status(400)
        .json({ success: false, message: "Password doesn't macthed" });
    }
